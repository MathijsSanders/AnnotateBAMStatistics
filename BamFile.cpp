...
//----------------------------------------------------------------        
bool CBamFile::Open(const string & sFileName,uint32_t dwMinQual,bool bCountDuplicates,bool bCountSupplementary)
{
    int             iMask;
    ...
    //----------------------------------------------------------------
    //Check if file is already open no need to close it
    //----------------------------------------------------------------
    
    if(this->sFileName.compare(sFileName)==0)
    {
        return true;
    }
    ...
    this->bCountSupplementary=bCountSupplementary;
...
int CBamFile::ReadRegion(bam1_t * pAlignment)
{
    ...
    {
        for(iRet=bam_iter_read(pBamFile,pBamRegion,pAlignment);iRet>=0 && (pAlignment->core.tid<0 || pAlignment->core.qual<dwMinQual || (pAlignment->core.flag&BAM_FSUPPLEMENTARY)!=0);iRet=bam_iter_read(pBamFile,pBamRegion,pAlignment));
    }
